...
namespace Meta.WitAi.Composer.Integrations
{
    public class WitComposerRequestHandler : IComposerRequestHandler
    {
        ...
        public void OnComposerRequestSetup(ComposerSessionData sessionData, VoiceServiceRequest request)
        {
            ...
            data[WitComposerConstants.ENDPOINT_COMPOSER_PARAM_CONTEXT_MAP] = sessionData.contextMap?.Data;
            var map = new ComposerContextMap(data);
            map?.SetData(sessionData.composer.contextMapEventKey,messageIsEmpty.ToString().ToLower());
            request.Options.QueryParams[WitComposerConstants.ENDPOINT_COMPOSER_PARAM_CONTEXT_MAP] = map?.GetJson();
